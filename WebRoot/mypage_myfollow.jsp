		...
		$("#page").Page(
				{
					...
					callBack : function(page) {
						...
						getmyfollow(Number($("#myfollow").children(
								"#follow_size").text()), page);
						window.scrollTo(0, 0);
					}
				});
		//if (nowpage != 1) {//主要为了样式
...
<script type="text/javascript">
	//获得关注列表
	function getmyfollow(myfollowcount, nowpage) {
		$
				.ajax({
					type : "post",
					url : "ajax/myfollowlist",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
					data : {//设置数据源
							...
							$("#topiccontainer")
									.append(
											'<div id="tab" class=""><div class="user"><div class="headimg"><img src="/upload/'+d[i].userid+'/headimg.jpg" onerror="'+defaultimg+'"></div><div class="headinfo"><li class="name"><a href="javascript:void(0)">'
													...
													+ '"><p class="tab_tiltle">'
													+ d[i].tiltle
													+ '</p><p class="tab_content">['+d[i].type+']'
													+ d[i].content
													...
													+ d[i].egg
													+ ')</a> <a class="unfollow" href="javascript:void(0);"><img src="icon/delete.png" class="iconimg"/>取消关注</a></div><div class="comment_below"><div class="comment_input" contenteditable="true" id="text'+d[i].id+'"></div><div class="comment_btn"><button>评论</button></div><div style="float: right;" class="emotion"><button>表情</button></div> <div style="clear: both;"></div><div class="sample_comment"></div></div></div>');
	...
	function Myfollow() {
		var totalcount_ = Number($("#follow_size").text());
		getmyfollow(totalcount_, 1);
		//pagetype = 1;
		initPage(totalcount_);//重新初始化我的topic的页
		//alert($(".mycontent_right").children(".common_headfont").text())
		$(".mycontent_right").children(".common_headfont_big").text("我的关注");
	}
	...
	function unfollowtopic(topicid) {
		$.ajax({
			type : "post",
			url : "ajax/unfollow",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
			data : {//设置数据源
				'topicid' : topicid
			},
			dataType : "json",//设置需要返回的数据类型
